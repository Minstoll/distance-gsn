...
from data.get_data import (
    load_zinc_homcount_dataset,
    load_zinc_subcount_dataset,
    load_zinc_subhom_dataset,
    load_zinc_resdist_dataset,
)
...
if __name__ == "__main__":
    ...
    wandb.init(project=conf["project"], group=conf["group"], name=conf["model"])
    ...
    if count_type == "homcounts" or count_type == "none":
        ...
        train_data, val_data, test_data, count_dim = load_zinc_homcount_dataset(
            name="ZINC", hom_files=count_files, idx_list=idx_list, root=data_dir
        )
    elif count_type == "subcounts":
        sub_file = count_files[0]
        print("loading subcounts")
        train_data, val_data, test_data, count_dim = load_zinc_subcount_dataset(
            name="ZINC", sub_file=sub_file, idx_list=idx_list, root=data_dir
        )
        ...
        train_data, val_data, test_data, count_dim = load_zinc_subhom_dataset(
            name="ZINC",
            hom_files=count_files,
            idx_list=idx_list,
            sub_file=sub_file,
            root=data_dir,
        )
    elif count_type == "resdist":
        print("loading resistance distances")
        rd_file = count_files[0]
        train_data, val_data, test_data, count_dim = load_zinc_resdist_dataset(
            name="ZINC", rd_file=rd_file, idx_list=idx_list, root=data_dir
        )  # count_dim here is length of idx_list in the config file. In this case the first n resistance distances
    else:
    ...
    for epoch in range(conf["epochs"]):
        epoch_train_curve = train(
            model_name, use_counts, model, device, train_loader, optimizer
        )
        ...
        valid_perf, epoch_val_loss = eval(
            model_name, use_counts, model, device, valid_loader
        )
        valid_curve.append(valid_perf)

        test_perf, epoch_test_loss = eval(
            model_name, use_counts, model, device, test_loader
        )
